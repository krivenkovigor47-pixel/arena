...
$(function() {
  ...
  function init(root) {
    ...
    var NODES = {
      ...
      tipsSection: root.querySelector("[data-wiz-tips-section]"),
  ...
  var CALC = {
    ...
    function renderTips() {
      ...
      if (!pool || !pool.length) {
        if (NODES.tipsSection) NODES.tipsSection.hidden = true;
        return;
      }

      if (NODES.tipsSection) NODES.tipsSection.hidden = false;
      ...
      for (var i = 0; i < 3; i++) {
        var t = pool[(tipOffset[m] + i) % pool.length];
      ...
      tipOffset[m] = (tipOffset[m] + 3) % pool.length;
    }
    ...
    function estimateText() {
      ...
      lines.push("");
      lines.push("Прошу проверить расчёт и сообщить наличие с доставкой.");
